# From https://bitbucket.org/amitibo/pyfisheye

import cv2
assert cv2.__version__[0] == '3', 'The fisheye module requires opencv version >= 3.0.0 not ' + cv2.__version__
    ...
    def calibrate(
        ...
        for img_index, (img, (ret, cb_2d_pts)) in enumerate(zip(imgs, rets)):
            ...
            if ret:
                ...
                #
                if self._verbose:
                    logging.info('OK' + os.path.split(fname)[1])
                ...
                if self._verbose:
                    logging.info('FAIL! ' + os.path.split(fname)[1])
